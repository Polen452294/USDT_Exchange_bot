...
log = logging.getLogger("nudge5")

router = Router()


@router.callback_query(F.data.startswith("n5_yes:") | F.data.startswith("n5_no:"))
async def on_nudge5_answer(call: CallbackQuery):
    ...
    answer = "YES" if action == "n5_yes" else "NO"
    now = datetime.utcnow()
    ...
    async with AsyncSessionLocal() as session:
        ...
        if req.nudge5_answered_at is not None:
            await call.answer("Ответ уже получен")
            return

        req.nudge5_answer = answer
        req.nudge5_answered_at = now
        await session.commit()

        try:
            await send_request_nudge_event(req, "nudge5", "yes" if answer == "YES" else "no")
        except Exception:
            pass
